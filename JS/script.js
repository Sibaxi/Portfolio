...
if(mobile){
    ...
    function handleOrientation(event) {
        if( beta < -30){ beta = -30}
        else if (beta > 50){ beta = 50}
        else if( gamma < -50){gamma = -50}
        else if ( gamma > 50){gamma = 50}
        else{
         beta  = event.beta;
         gamma = event.gamma;
...
p.setup = function() {
    ...
    canvas = p.createCanvas(clientWidth + clientWidth*0.2, clientHeight*0.8);
...
p5.Vector.prototype.frame = function()  {
  ...
  if (this.ind_x == 0 || this.ind_x == rows - 1 || this.ind_y == 0 || this.ind_y == columns - 1  ) {
    return;
  }
...
p.setup = function() {
  p.createCanvas(clientWidth, clientHeight/5);
}

let transition = p._userNode ;
p.draw = function() {
  ...
  for (let x = 0; x <= clientWidth+15; x += 40) {
    // Calculate a y value according to noise, map to

    // Option #1: 2D Noise
    let y = p.map(p.noise(xoff, yoff), 0, 1, clientHeight/10, clientHeight/5);
  ...
  yoff += 0.005;
  p.curveVertex(clientWidth,clientHeight/8);
    if ( transition == 'secondTransition' ) {
       p.curveVertex(clientWidth, 0);
       p.curveVertex(0, 0);
  }
    else {
        p.curveVertex(clientWidth, p.height);
...
let firstWave = new p5(wave,'firstTransition');
let thirdWave = new p5(wave,'thirdTransition');
let secondWave = new p5(wave, 'secondTransition');
...
// Home transition
    ...
    for (let j = 0; j < hiddenOnHome.length ; j++){
        ...
        hiddenOnHome[j].className = hiddenOnHome[j].className.replace('hiddenHome','showHome');    
    }
////////////////////////////////////////////////////////////////////////////////////
...
function scrollPosition(){
    ...
    for (let i = 0; i < segments.length ; i++){
       ...
       else if (currentPosition > segments[1].offsetTop-400  && currentPosition <= segments[2].offsetTop -400){
          activeLink[1].classList.add('activePage');
       }
       else if (currentPosition > segments[2].offsetTop -400 && currentPosition <= segments[3].offsetTop -400){
...
const easing = 0.05;
if(mobile == false){
    window.onmouseout = function (){ cursor.style.display = 'none'};
    window.onmouseover = function (){ cursor.style.display = 'block'};
...
let workbox = document.querySelectorAll('.workBox');
 ...
 for (let i = 0; i < workbox.length ; i++){
     ...
     workbox[i].addEventListener("mouseover", function( event ) { 
     ...
     workbox[i].addEventListener("mouseout", function( event ) {
     for (let j = 0; j < workbox.length ; j++){
               animateLine[j].style.strokeDashoffset = '0';
           } 
    }, false);
